...
class Cache{
public:
	bool validate(const Request & client_request, Response& cache_response, string& message);
...
bool Cache::validate(const Request & client_request, Response& cache_response, string& message){
...
bool Cache::find_block(const Request & client_request, Response& cache_response){
	...
	if(iter==caches.end()){
		...
		auto list_iter  = std::find(block_list.begin(), block_list.end(), client_request.get_sub_url());
